...
## Digit span plots
cbs_wm %>% ggplot(mapping= aes(x=cohort, y= Digit.Span_Single.Score.Raw,fill= cohort)) + geom_boxplot() + xlab("Group")+ ylab("Digit Span Score")+ labs(fill= "Group", title = "S1: Working Memory- Digit Span Scores")+
...
#
cbs_wm %>% ggplot(mapping= aes(x=cohort, y= Monkey.Ladder_Single.Score.Raw,fill= cohort)) + geom_boxplot() + xlab("Group")+ ylab("Monkey Ladder Score")+ labs(fill= "Group", title = "S2: Working Memory- Monkey Ladder Scores")+
    stat_boxplot(geom='errorbar', linetype=1, width=0.1)+ 
    geom_boxplot(outlier.shape=1)+ 
    geom_jitter(color="black", size=0.4, alpha=0.9) 
