...
ggplot(feature_match_accuracy, aes(x = condition, y = accuracy_rate_per_condition)) +
    ...
    geom_line(aes(group = Subject_ID ), col = "darkgrey", size = 0.25, alpha=0.4) + # plot individual data
    ...
    labs(title= "C", x = "Condition", y = "Accuracy Rate") + theme_minimal()
...
ggplot(polygons_accuracy, aes(x = condition, y = accuracy_rate_per_condition)) +
    ...
    stat_summary(fun.data = "mean_cl_normal", size = 0.40, color= "darkred") + # add mean and between-subjects
    # 95% CIs; note these are not accurate for a within-subjects comparison
